...
function SignupStep1({ handleActions }) {
  ...
  const submitPhone = async () => {
    setLoading(true);
    try {
      console.log('Checking phone number:', phoneNum.value);
      ...
      if (isRegistered) {
        ...
        console.log('Sending OTP to:', phoneNum.value);
      ...
      setLoading(false);
      toastMessage("Something went wrong here.", "error");
    }
  };
  ...
  const onSubmit = () => {
    ...
    setPhoneNum({ ...phoneNum, isError, errorMsg });
